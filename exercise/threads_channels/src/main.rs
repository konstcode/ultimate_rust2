...
fn main() {
    ...
    // main thread loop hrough several values and print each out and then send it to the channel.
    ...
    sleep_ms(50); // Make sure Thread A has time to get going before we spawn Thread B
    ...
    for i in 0..5 {
        println!("Main sent {}", i);
        let _ = tx.send(format!("Data form main {}", i));
    }
